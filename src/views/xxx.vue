      ...
      <div class="message-box  h-[85%] overflow-y-scroll scrollbar scrollbar-thin scrollbar-thumb-slate-600"  >
        <template v-for="item in chatroomRefList" :key="item.id">
            ...
            <div class="message-other  h-36 flex flex-wrap relative" >
              <div class="flex h-20 max-w-[80%] min-w-[60%] m-auto items-center   absolute  shadow-lg" :class="[{ ' left-8  rounded-tl-lg rounded-tr-lg rounded-br-lg bg-white': !isSelf(item.username) } ,{ ' right-8 rounded-tl-lg rounded-tr-lg rounded-bl-lg flex-row-reverse bg-slate-200': isSelf(item.username) }]">
                <div class="rounded-full bg-slate-300 w-16 h-16 self-auto flex justify-center " :class="[{ 'ml-3': !isSelf(item.username) } ,{ 'mr-3': isSelf(item.username)}]"
                >
