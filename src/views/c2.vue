    ...
    <div class="right-side h-screen  grow relative">
      <div class="message-box  h-[85%] overflow-y-scroll">
        <template v-for="item in chatroomRefList" :key="item.id">
          <template v-if="item.username !== tempUsername">
            <div
              class="message-other  h-36 flex flex-wrap relative"
            >
              <div
                class="flex h-20 max-w-[80%] min-w-[60%] m-auto items-center bg-white rounded-tl-lg rounded-tr-lg rounded-br-lg absolute left-8 shadow-lg"
              >
                <div
                  class="rounded-full bg-slate-300 w-16 h-16 self-auto ml-3"
                ></div>
                <div class="ml-3 w-11/12">
                  <div class="text-slate-200">{{ item.username }}</div>
          ...
          <template v-if="item.username == tempUsername">
            <div class="message-self h-36 flex flex-wrap relative">
              <div
                class="flex h-20 max-w-[80%] min-w-[60%] m-auto items-center bg-white rounded-tl-lg rounded-tr-lg rounded-bl-lg flex-row-reverse absolute right-8 shadow-lg"
              >
                <div
      ...
      <div class="type-box absolute bottom-8 w-full h-16">
        <div class="flex h-full w-[92.6667%] m-auto">
          <div class="rounded-full bg-slate-500 w-16 h-16 self-auto"></div>
          <input
            type="text"
